...
@Component({
  selector: 'app-editor-fabric',
  templateUrl: './editor-fabric.component.html',
  styleUrls: ['./editor-fabric.component.scss']
})
export class EditorFabricComponent implements OnInit, AfterViewInit, OnDestroy {
  @ViewChild('htmlCanvas') htmlCanvas: ElementRef;
  @Input('urlImageAvatar') urlImageAvatar: string;
  ...
  viewDevice() {
    if (window.innerWidth <= 320) {
      this.canvas.setWidth(160);
      this.canvas.setHeight(160);
    }
  ...
  ngAfterViewInit(): void {
    ...
    //mio papu
    if (this.urlImageAvatar) {
      this.toDataURL(this.urlImageAvatar, (dataUrl) => {
        this.addImageOnCanvas(dataUrl);
      });
    }
  ...
  //de imagen a base 64 super importante
  toDataURL(url, callback) {
    var xhr = new XMLHttpRequest();
    //para evitar el cacheo de imagenes
  ...
  download_img() {
    ...
    var imageURI = this.canvas.toDataURL({ format: 'image/jpg' });
  ...
  readUrl(event) {
    //tl this.url es una imagen base64 ojo con ese detalle
  ...
  rasterize() {
    ...
    return fetch(image.src)
      .then(res => res.blob())
      .then(blob => {
        const file = new File([blob], `avatar${new Date().getTime()}.png`, blob)
        return file
      })
    /* const w = window.open('');
